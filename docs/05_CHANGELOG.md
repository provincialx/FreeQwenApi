...
| 47 | Cross-account token rotation breaking existing chats | Each Qwen chat belongs to the account that created it. `createChatV2()` rotated accounts via round-robin `getAvailableToken()`, so the message sender could pick a different account than the creator → "not exist" on other accounts' chats. Fixed: (1) New `chatTokenOwner` Map binds qwenChatId → accountId at creation time. (2) `resolveAuthToken()` tries owner's token first before falling back to rotation. (3) `getTokenById(id)` in tokenManager.js for targeted account lookup. (4) GC caps chatTokenOwner at 500 entries, evicts unreferenced chats. | qwenApi.js, chatSession.js, tokenManager.js |
